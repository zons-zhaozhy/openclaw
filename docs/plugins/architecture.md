...
- runtime hooks: `resolveDynamicModel`, `prepareDynamicModel`, `normalizeResolvedModel`, `capabilities`, `prepareExtraParams`, `wrapStreamFn`, `formatApiKey`, `refreshOAuth`, `buildAuthDoctorHint`, `isCacheTtlEligible`, `buildMissingAuthMessage`, `suppressBuiltInModel`, `augmentModelCatalog`, `isBinaryThinking`, `supportsXHighThinking`, `resolveDefaultThinkingLevel`, `isModernModelRef`, `prepareRuntimeAuth`, `resolveUsageAuth`, `fetchUsageSnapshot`, `buildReplayPolicy`, `sanitizeReplayHistory`, `validateReplayTurns`, `contributeResolvedModelCompat`, `createEmbeddingProvider`, `createStreamFn`, `normalizeConfig`, `normalizeModelId`, `normalizeToolSchemas`, `inspectToolSchemas`, `normalizeTransport`, `resolveConfigApiKey`, `resolveReasoningOutputMode`, `buildUnknownModelHint`, `applyNativeStreamingUsageCompat`, `resolveSyntheticAuth`, `shouldDeferSyntheticProfileAuth`
...
| 25  | `contributeResolvedModelCompat`   | Compat flags for vendor models behind another transport                                  | Provider recognizes its models behind OpenRouter/proxy                               |
| 26  | `createEmbeddingProvider`         | Plugin-owned embedding provider factory                                                  | Provider has its own embedding endpoint                                              |
| 27  | `createStreamFn`                  | Fully custom transport factory                                                           | Provider needs a custom wire protocol                                                |
| 28  | `normalizeConfig`                 | Provider-specific `models.providers.<id>` cleanup                                        | Provider needs config normalization beyond core                                      |
| 29  | `normalizeModelId`                | Alias cleanup for preview/legacy model IDs                                               | Provider has model ID aliases                                                        |
| 30  | `normalizeResolvedModel`          | (already listed at #4)                                                                   |                                                                                      |
| 31  | `normalizeToolSchemas`            | Tool-schema cleanup before runner registration                                           | Provider has tool-schema transport quirks                                            |
| 32  | `inspectToolSchemas`              | Transport-specific schema diagnostics                                                    | Provider wants custom schema warnings                                                |
| 33  | `normalizeTransport`              | API/baseUrl normalization before model assembly                                          | Provider needs transport-family cleanup                                              |
| 34  | `resolveConfigApiKey`             | Auth from env markers (AWS/GCP) instead of API key                                       | Provider uses non-standard auth env markers                                          |
| 35  | `resolveReasoningOutputMode`      | Tagged reasoning vs native structured fields                                             | Provider requires tagged reasoning output                                            |
| 36  | `buildUnknownModelHint`           | Custom hint when catalog lookup fails                                                    | Provider wants specific recovery guidance                                            |
| 37  | `applyNativeStreamingUsageCompat` | Native streaming usage compat for config providers                                       | Provider opts specific URLs into streaming usage                                     |
| 38  | `resolveSyntheticAuth`            | Synthetic auth profile resolution                                                        | Provider needs synthetic profile construction                                        |
| 39  | `shouldDeferSyntheticProfileAuth` | Defer synthetic profile auth to runtime                                                  | Provider needs deferred auth resolution                                              |
